...
- **Modelo, firmware, RAM e SD (agente ≥ 1.1.0):** `nintendo_device_info` devolve `model`, `firmware`, `ram_bytes`, `app_memory`, `system_memory` e `sd_card` (`total`/`free` em bytes). O que o console não consegue medir vem `null` e aparece em `unavailable`; nada é adivinhado. Agentes mais antigos respondem só com o que já tinham.
