...
## Applets da Nintendo (Notas de jogo etc.) como hospedeiro — ideia do usuário
Notas de jogo, Lista de amigos, Notificações e Navegador são **applets** iniciados pelo HOME com o jogo **suspenso** — o jogo fica congelado na memória e volta de onde parou; **não rodam em paralelo com o jogo em execução** ([HOME Menu — Fandom](https://nintendo3ds.fandom.com/wiki/HOME_Menu), [Home Menu — 3dbrew](https://www.3dbrew.org/wiki/Home_Menu), [NS and APT Services — 3dbrew](https://www.3dbrew.org/wiki/NS_and_APT_Services)). Portanto **não dão log ao vivo com o jogo rodando**. Mas resolveriam metade da dor: um agente hospedado num applet (ex.: substituindo o código de um applet que o usuário não usa, via `/luma/titles/<id>/`, pois `enable_game_patching = 1`) permitiria **pausar o jogo → ler o log → voltar**, sem fechar o jogo. Incertezas **não verificadas**: memória disponível ao applet, acesso a `soc:U` (ACL do applet), qual applet é seguro substituir. Os únicos componentes realmente concorrentes com o jogo são sysmodules (BOSS, NEWS, friends…), com o problema já descrito de sequestrar um módulo de sistema.
