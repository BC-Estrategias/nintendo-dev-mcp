...
static size_t finish(uint8_t *out, size_t cap, const ndp_header *req, uint8_t kind, uint16_t status,
                     const ndp_tlv_w *w) {
  ndp_header h;
...
static size_t err(uint8_t *out, size_t cap, const ndp_header *req, uint16_t status, const char *detail,
                  int with_supported) {
  ndp_tlv_w w;
  ...
  return finish(out, cap, req, NDP_KIND_ERR, status, &w);
}
...
static size_t do_hello(ndp_agent *a, const ndp_header *req, const uint8_t *pl, uint8_t *out, size_t cap) {
  ...
  if (!ndp_tlv_find(pl, req->payload_len, NDP_TAG_PROTOCOL, &vmin, &lmin) ||
    ...
    return err(out, cap, req, NDP_ST_BAD_REQUEST, "missing field", 0);
  if (lmin != 2 || lmax != 2 || lnon != 16) return err(out, cap, req, NDP_ST_BAD_REQUEST, "bad field size", 0);
  lo = ndp_tlv_get_u16(vmin);
  hi = ndp_tlv_get_u16(vmax);
  if (lo > hi) return err(out, cap, req, NDP_ST_BAD_REQUEST, "protocol_min > protocol_max", 0);
  chosen = hi < NDP_PROTOCOL_VERSION ? hi : NDP_PROTOCOL_VERSION;
  if (chosen < (lo > NDP_PROTOCOL_VERSION ? lo : NDP_PROTOCOL_VERSION))
    return err(out, cap, req, NDP_ST_UNSUPPORTED_PROTOCOL, "no common protocol version", 1);
  ...
  return finish(out, cap, req, NDP_KIND_RES, NDP_OK, &w);
}
...
static size_t do_ping(const ndp_header *req, const uint8_t *pl, uint8_t *out, size_t cap) {
  ...
  if (!ndp_tlv_find(pl, req->payload_len, NDP_TAG_PING_NONCE, &n, &ln) || ln != 8)
    return err(out, cap, req, NDP_ST_BAD_REQUEST, "ping_nonce required", 0);
  if (cap < NDP_HEADER_SIZE) return 0;
  ndp_tlv_w_init(&w, out + NDP_HEADER_SIZE, cap - NDP_HEADER_SIZE);
...
size_t ndp_agent_handle(ndp_agent *a, const ndp_header *req, const uint8_t *payload, uint8_t *out,
  ...
  if (req->version != NDP_PROTOCOL_VERSION)
    return err(out, cap, req, NDP_ST_UNSUPPORTED_PROTOCOL, "unsupported frame version", 1);
  if (req->kind != NDP_KIND_REQ) return err(out, cap, req, NDP_ST_BAD_REQUEST, "expected REQ", 0);
  if (!ndp_tlv_validate(payload, req->payload_len))
    return err(out, cap, req, NDP_ST_BAD_REQUEST, "malformed payload", 0);
  if (req->command == NDP_CMD_HELLO) return do_hello(a, req, payload, out, cap);
  if (!a->hello_done) return err(out, cap, req, NDP_ST_HELLO_REQUIRED, "send HELLO first", 0);
  if (req->command == NDP_CMD_PING) return do_ping(req, payload, out, cap);
  return err(out, cap, req, NDP_ST_UNSUPPORTED_COMMAND, "unknown command", 0);
}
