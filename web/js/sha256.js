/* SHA-256 and HMAC-SHA-256 in plain JavaScript. The console's page is served over http://, where the browser does not
 * offer crypto.subtle, so the primitives are implemented here and verified against the same vectors as the C and
 * TypeScript implementations (docs/protocol/test-vectors). */
(function () {
  "use strict";
  ...
  class Sha256 {
    ...
    #block(p, o) {
      ...
      for (let i = 0; i < 16; i++, o += 4) w[i] = (p[o] << 24) | (p[o + 1] << 16) | (p[o + 2] << 8) | p[o + 3];
      for (let i = 16; i < 64; i++) {
    ...
    update(data) {
      const p = data instanceof Uint8Array ? data : new Uint8Array(data);
      let i = 0;
      this.total += p.length;
      if (this.bufLen) {
        while (i < p.length && this.bufLen < 64) this.buf[this.bufLen++] = p[i++];
        if (this.bufLen < 64) return this;
        this.#block(this.buf, 0);
        this.bufLen = 0;
      }
      for (; i + 64 <= p.length; i += 64) this.#block(p, i);
      while (i < p.length) this.buf[this.bufLen++] = p[i++];
      return this;
    }
    ...
    digest() {
      ...
      for (let i = 0; i < 8; i++) odv.setUint32(i * 4, this.h[i] >>> 0);
      return out;
    }
  }
  ...
  function sha256(...parts) {
    const s = new Sha256();
    for (const p of parts) s.update(p);
    return s.digest();
  }
  ...
  function hmacSha256(key, ...parts) {
    ...
    const inner = new Sha256().update(ipad);
    for (const p of parts) inner.update(p);
    return sha256(opad, inner.digest());
  }

  NDP.Sha256 = Sha256;
  NDP.sha256 = sha256;
  NDP.hmacSha256 = hmacSha256;
})();
